...
export default function Geoscape() {
    ...
    return (
        <>
            <Canvas className={"canvas"} style={{height: "50vh"}} pixelRatio={window.devicePixelRatio}
                    camera={camera}>
                <ambientLight color={"0xFFFFFF"} intensity={0.3}/>
                <directionalLight color={"0xFFFFFF"} intensity={1} position={[0, 2000, -2800]}/>
                <spotLight color={"0xd30491"} intensity={20} distance={3000} angle={Math.PI}
                           position={[0, 1500, -1300]}/>
                <Terrain/>
            </Canvas>
        </>
    )
